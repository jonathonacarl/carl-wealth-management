...
def get_tech_ind(data):
    # data = dropna(data)
    data["KAMA_3"] = KAMAIndicator(data["Close"], window=3, pow1=2, pow2=30).kama()
    data["KAMA_20"] = KAMAIndicator(data["Close"], window=20, pow1=2, pow2=30).kama()
    return data
...
def make_chart(data_plt, series="TQQQ", type="DAILY_YTD"):
    if type == "DAILY_YTD":
        ...
        start_date = pd.to_datetime(str(data_plt["Date"].max().year))
        wclose = data_plt.groupby(
            [data_plt["Date"].dt.year, data_plt["Date"].dt.week], as_index=False
        )[["Date", "Close"]].last()
        wopen = data_plt.groupby(
            [data_plt["Date"].dt.year, data_plt["Date"].dt.week], as_index=False
        )["Open"].first()
        whigh = data_plt.groupby(
            [data_plt["Date"].dt.year, data_plt["Date"].dt.week], as_index=False
        )["High"].max()
        wlow = data_plt.groupby(
            [data_plt["Date"].dt.year, data_plt["Date"].dt.week], as_index=False
        )["Low"].min()
        wvol = data_plt.groupby(
            [data_plt["Date"].dt.year, data_plt["Date"].dt.week], as_index=False
        )["Volume"].sum()
    ...
    kama = [
        fplt.make_addplot(data_plt["KAMA_3"], linestyle="dotted", color="blue"),
        fplt.make_addplot(data_plt["KAMA_20"], linestyle="dotted", color="black"),
    ]
    fplt.plot(
        data_plt[["Open", "High", "Low", "Close", "Volume"]],
        type="candle",
        volume=True,
        style="yahoo",
        addplot=kama,
        tight_layout=True,
        savefig="../public/images/" + type + "_" + series + ".png",
    )
